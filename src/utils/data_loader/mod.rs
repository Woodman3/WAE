...
pub(crate) struct Loader{
    ...
    skill_table:Value,
    enemy_database:HashMap<String,Value>,
}
...
impl Loader{
    pub(crate) fn new<P: AsRef<Path>>(path: P) -> Result<Loader> {
        ...
        let enemy_database_j = load_json_file(path.as_ref().join("levels/enemydata/enemy_database.json"))?;
        let enemy_database_v =from_value::<Vec<OfficalEnemy>>(enemy_database_j["enemies"])?;
