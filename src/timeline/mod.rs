...
pub(super) fn read_timeline(c: &Config) -> Result<(VecDeque<EventWithTime>, u64)> {
    ...
    for v in c.hostile["timeline"].as_array().unwrap() {
        ...
        let e: Rc<dyn Event> = match op.as_str() {
            "Place" => Rc::new(EnemyPlaceEvent::new(v)?),
...
pub(crate) fn action_to_event(v: &Value) -> Result<Rc<dyn Event>> {
    ...
    let e: Rc<dyn Event> = match op.as_str() {
        "Deploy" => Rc::new(OperatorDeployEvent::new(v)?),
        "Retreat" => Rc::new(OperatorRetreatEvent {
            operator_key: v[2]
                .as_str()
        ...
        "Skill" => Rc::new(serde_json::from_value::<OperatorSkillEvent>(v[2].clone())?),
        _ => return Err(ConfigParseError("unknown op in timeline".into()).into()),
    };
    Ok(e)
}
